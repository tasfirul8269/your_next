...
class CustomerRepository extends Repository
{
    ...
    {
        return 'Frooxi\Customer\Contracts\Customer';
    }
        ...
        if (isset($data[$type])) {
            ...
            foreach ($data[$type] as $imageId => $image) {
                ...
                if ($request->hasFile($file)) {
                    if ($customer->{$type}) {
                        Storage::disk(config('filesystems.default'))->delete($customer->{$type});
                    }

                    $customer->{$type} = cloudinary_upload($request->file($file), 'customers', (string) $customer->id);
                    $customer->save();
                }
            }
        } else {
