...
@pushOnce('scripts')
    ...
    <script type="module">
        app.component('v-range-slider', {
            ...
            data() {
                const allowedMinRange = parseFloat(this.defaultAllowedMinRange ?? 0);
                const allowedMaxRange = parseFloat(this.defaultAllowedMaxRange ?? 100);

                return {
                    gap: Math.max((allowedMaxRange - allowedMinRange) * 0.10, 0),
                    ...
                    minRange: parseFloat(this.defaultMinRange ?? allowedMinRange),

                    maxRange: parseFloat(this.defaultMaxRange ?? allowedMaxRange),
                };
            },
            ...
            methods: {
                ...
                handleProgressBar() {
                    ...
                    const rangeSpan = this.allowedMaxRange - this.allowedMinRange || 1;
                    const minProgress = ((this.minRange - this.allowedMinRange) / rangeSpan) * 100;
                    const maxProgress = ((this.maxRange - this.allowedMinRange) / rangeSpan) * 100;

                    this.$refs.progress.style[direction] = minProgress + '%';

                    this.$refs.progress.style[direction == 'left' ? 'right' : 'left'] = 100 - maxProgress + '%';
                },
