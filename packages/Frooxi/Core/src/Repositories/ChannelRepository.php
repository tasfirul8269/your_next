...
class ChannelRepository extends Repository
{
    ...
    {
        return 'Frooxi\Core\Contracts\Channel';
    }
    ...
    public function uploadImages($data, $channel, $type = 'logo')
    {
        if (request()->hasFile($type)) {
            ...
            $channel->{$type} = cloudinary_upload($file, 'channels', (string) $channel->id, $type, false);

            $channel->save();
        } else {
            if (! isset($data[$type])) {
                if (! empty($data[$type])) {
                    Storage::disk(config('filesystems.default'))->delete($channel->{$type});
                }
