...
namespace Frooxi\Category\Repositories;

use Frooxi\Category\Contracts\Category;
use Frooxi\Category\Models\CategoryTranslationProxy;
        ...
        if (isset($data[$type])) {
            foreach ($data[$type] as $imageId => $image) {
                ...
                if (request()->hasFile($file)) {
                    if ($category->{$type}) {
                        Storage::disk(config('filesystems.default'))->delete($category->{$type});
                    }

                    $category->{$type} = cloudinary_upload(request()->file($file), 'categories', (string) $category->id, $type);
