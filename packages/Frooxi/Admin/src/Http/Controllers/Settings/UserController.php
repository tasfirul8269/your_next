...
use Frooxi\Admin\DataGrids\Settings\UserDataGrid;
use Frooxi\Admin\Http\Controllers\Controller;
use Frooxi\Admin\Http\Requests\UserForm;
...
class UserController extends Controller
{
    ...
    public function store(UserForm $request): JsonResponse
    {
        ...
        if (request()->hasFile('image')) {
            $admin->image = cloudinary_upload(current(request()->file('image')), 'admins', (string) $admin->id);
        ...
        if (request()->hasFile('image')) {
            ...
            if (! request()->has('image.image')) {
                if (! empty(request()->input('image.image'))) {
                    Storage::disk(config('filesystems.default'))->delete($admin->image);
                }
