...
class ProductImage
{
    ...
    public function getGalleryImages($product)
    {
        ...
        foreach ($product->images as $image) {
            // FAST: Skip existence check for Cloudinary (avoids 2.7s API call)
            // Only check for local storage where files might be missing
            if (config('filesystems.default') !== 'cloudinary' && ! Storage::has($image->path)) {
                continue;
            }
    ...
    {
        // FAST: Use direct Cloudinary URL generation (no API calls)
        if (config('filesystems.default') === 'cloudinary') {
            return [
                'small_image_url' => cloudinary_url($path, ['w' => 300, 'q' => 'auto']),
                'medium_image_url' => cloudinary_url($path, ['w' => 600, 'q' => 'auto']),
                'large_image_url' => cloudinary_url($path, ['w' => 900, 'q' => 'auto']),
                'original_image_url' => cloudinary_url($path),
            ];
        }
    ...
    private function getFallbackImageUrls(): array
    {
        $smallImageUrl = core()->getConfigData('catalog.products.cache_small_image.url')
                        ...
                        : yournext_asset('images/small-product-placeholder.webp', 'shop');
        ...
        $mediumImageUrl = core()->getConfigData('catalog.products.cache_medium_image.url')
                        ...
                        : yournext_asset('images/medium-product-placeholder.webp', 'shop');
        ...
        $largeImageUrl = core()->getConfigData('catalog.products.cache_large_image.url')
                        ...
                        : yournext_asset('images/large-product-placeholder.webp', 'shop');

        return [
            'small_image_url' => $smallImageUrl,
            'medium_image_url' => $mediumImageUrl,
            'large_image_url' => $largeImageUrl,
            'original_image_url' => yournext_asset('images/large-product-placeholder.webp', 'shop'),
        ];
    }
