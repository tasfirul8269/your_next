...
| Area | Status |
|---|---|
| Docker | **Added (June 2026)** — `docker-compose.dev.yml` (dev: app + Vite HMR + MySQL + Mailpit + Adminer) and `docker-compose.prod.yml` (prod: app + Nginx + MySQL, reverse-proxy-friendly). See [DEPLOYMENT.md](DEPLOYMENT.md). The Docker setup reflects what the app actually uses: file cache, sync/DB queue, and MySQL-based search (no Redis/Elasticsearch). |
